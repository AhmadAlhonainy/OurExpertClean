...
- Added admin control to delete experiences: `DELETE /api/admin/experiences/:id` (safely cascade deletes all related bookings, conversations, availability)
...
- Added `deleteBooking` method to storage layer for cascade operations
...
- Delete cascade logic: Deactivates conversations → releases availability → deletes bookings → deletes experience
